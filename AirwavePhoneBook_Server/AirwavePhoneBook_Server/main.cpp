...
int main() {
	...
	try {
		...
		for (;;) {
			...
			write(socket, buffer("\t4) Delete a contact\n"));
			write(socket, buffer("\t5) View the phonebook\n"));
			write(socket, buffer("\t6) Close the connection\n"));
			...
			case MENU::NEW_CONTACT:
			{
				...
				std::string contact_relationship;
				...
				if (contact_type == "BUSINESS" || contact_type == "PERSONAL") {
					...
					write(socket, buffer("\nPlease enter relationship to contact (business/personal):\n"));
					contact_relationship = read_remote_input(socket, error);
					...
					phonebook.add_contact(contact_type, contact_name, contact_number, contact_relationship, contact_info);
				}
				else
				...
				if (contact_type == "BUSINESS" || contact_type == "PERSONAL") {
					...
					if (found_contact == nullptr) {
						...
						if (contact_type == "BUSINESS") {
							// If the contact type is business, print the company and business relationship
							write(socket, buffer(((Business*)found_contact)->get_business_relationship()));
							write(socket, buffer("\n"));
							write(socket, buffer(((Business*)found_contact)->get_company()));
						}
						else if (contact_type == "PERSONAL") {
							// If the contact type is personal, print the nickname and personal relationship
							write(socket, buffer(((Personal*)found_contact)->get_personal_relationship()));
							write(socket, buffer("\n"));
					...
					// Otherwise, tell the user they entered an invalid contact type
					write(socket, buffer("\nThat is not a valid contact type.\n"));
				break;
			}
			// Sort contacts
			...
			case MENU::DELETE_CONTACT:
			{
				...
				// Only delete a contact if the user enters a valid contact type
				if (contact_type == "BUSINESS" || contact_type == "PERSONAL") {
					write(socket, buffer("\nPlease enter a search term corresponding to the contact you would like to delete:\n"));
					search_term = read_remote_input(socket, error);

					// Delete the contact
					write(socket, buffer("\nDeleting contact...\n"));
					phonebook.erase_contact(contact_type, search_term);
					write(socket, buffer("\nDone.\n"));
				}
				else 
					// Otherwise, tell the user they entered an invalid contact type
...
void view_phone_book(tcp::socket& connection, error_code& error, Directory* list) {
	...
	for (unsigned int i = 0; i < display_list->size(); i++) {
		...
		if (user_choice == "BUSINESS") {
			write(connection, buffer(((Business*)contact_element)->get_business_relationship()));
			write(connection, buffer("\n"));
			write(connection, buffer(((Business*)contact_element)->get_company()));
		}
		else if (user_choice == "PERSONAL") {
			write(connection, buffer(((Personal*)contact_element)->get_personal_relationship()));
			write(connection, buffer("\n"));
